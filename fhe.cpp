...
mp_int **fhe_band_integers(mp_int **integer1, mp_int **integer2) {
    mp_int **result;
    ...
    for (int i = 0; i < FHE_INTEGER_BIT_WIDTH; i++) {
        assert(integer1[i] != NULL && integer2[i] != NULL);
        result[i] = fhe_and_bits(integer1[i], integer2[i]);
    }

    return result;
}

mp_int **fhe_bxor_integers(mp_int **integer1, mp_int **integer2) {
    mp_int **result;
    ...
    for (int i = 0; i < FHE_INTEGER_BIT_WIDTH; i++) {
        assert(integer1[i] != NULL && integer2[i] != NULL);
        result[i] = fhe_xor_bits(integer1[i], integer2[i]);
    }
...
int main(int argc, char **argv) {
    ...
    // Bit-wise XOR after Bit-wise AND: op1 & op2 ^ op2 = ~op1 & op2 
    (void)printf("\n");
    for (int i = 0; i < 16; i++) {
	fhe_integer result, op1, op2;
	op1 = fhe_random(FHE_INTEGER_BIT_WIDTH);
	op2 = fhe_random(FHE_INTEGER_BIT_WIDTH);
	
	result = fhe_decrypt_integer(
    tmp1 = fhe_band_integers(
    fhe_encrypt_integer(~op1),
    fhe_encrypt_integer(op2)
    ));

	DESTROY_ENCRYPTED_INTEGER(tmp1);
	retval |= !(ok = (result == (fhe_integer)FHE_MASK(op1 & op2 ^ op2)));

	(void)printf("~0x%1$0*6$x & 0x%2$0*6$x = 0x%3$0*6$x (0x%4$0*6$x) %5$s\n",
		op1,
		op2,
		result,
        ~op1 & op2,
